...
export class ChatController {
  ...
  @ApiOperation({
    ...
    description: [
      ...
      '`agent.step`, `plan.created`, `function.started`, `function.completed`,',
      '`reflection`. These name functions and echo extracted parameters, so an',
      'end-user surface should use a key without that scope.',
      '',
      'The agent works in steps: it calls one function, reads the result, and',
      'decides what is next. `agent.step` is emitted once per step and carries',
      'its number, so a lookup followed by an action reads as two decisions.',
      '`plan.created` is emitted alongside it for each step, so a client written',
      'against the single-shot pipeline keeps working — it will now simply see',
      'one per step rather than one per run.',
    ].join('\n'),
  })
